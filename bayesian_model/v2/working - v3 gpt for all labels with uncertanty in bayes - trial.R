...
temperature <- 0.9  # Increasing the temperature makes the output probabilities closer
...
total_df = total_df %>% 
  mutate(gpt_product_type_confidence = case_when(gpt_product_type_confidence == "High" ~ 3,
         ...
         gpt_image_reflection = case_when(gpt_image_reflection == "High" ~ 3,
                                          ...
                                          TRUE ~ 0))
unique(total_df$gpt_product_type_confidence)
...
confidenceB = as.integer(total_df$gpt_product_type_confidence)
...
# data uncertainty
sigma_data = total_df$avg_entropy_product
# model uncertainty
tau_muA1 = total_df$avg_var_product
...
model_code = "
    model{
      for (i in 1:N) {
        ...
        # # Adjusting means with model uncertainty
        # muA1[i] ~ dnorm(0, 1 / tau_muA1[i])
        # muA0[i] ~ dnorm(0, 1 / tau_muA1[i]) # Assuming model uncertainty affects both classes similarly
        # for (k in 1:L) {
        #   muA[i,k] <- ifelse( truelabel[i]==k , muA1[i], muA0[i] )
        # }
        ...
        for (k in 1:L) {
          # Adjusting precision with data uncertainty
          logitscoresA[i,k] ~ dnorm(muA[i,k], 1 / (sigma_data[i] * sigma_data[i]))
