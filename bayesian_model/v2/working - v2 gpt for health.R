...
###### BAYESIAN MODEL FOR HEALTH #######
# process big model file
all_imgs_results_big_model = all_imgs_results_big_model3

# Select columns that start with 'HealthStatus_'
health_cols <- grep("HealthStatus_", names(all_imgs_results_big_model), value = TRUE)
health_cols <- sub("HealthStatus_", "", health_cols)

# Extract true label for health
all_imgs_results_big_model$label_health <- sapply(strsplit(all_imgs_results_big_model$label, "_"), function(x) if(length(x) >= 2) x[4] else NA)
table(all_imgs_results_big_model$label_health)
sum(is.na(all_imgs_results_big_model$label_health))

# Remove 'healthStatus_' prefix from column names
names(all_imgs_results_big_model) <- sub("HealthStatus_", "", names(all_imgs_results_big_model))

# Select label, group, logits and true label
all_imgs_results_big_model = all_imgs_results_big_model[,c("label","group",health_cols,"label_health", "avg_var_health", "avg_entropy_health")]
...
sum(is.na(all_imgs_results_gpt$gpt_health)) > 0
sum(is.na(all_imgs_results_gpt$gpt_image_reflection)) > 0
sum(is.na(all_imgs_results_gpt$gpt_image_clarity)) > 0
sum(is.na(all_imgs_results_gpt$gpt_health_confidence)) > 0

# keep valid healths
unique(all_imgs_results_gpt$gpt_health)
dim(all_imgs_results_gpt)
all_imgs_results_gpt = all_imgs_results_gpt[all_imgs_results_gpt$gpt_health %in% health_cols,]
unique(all_imgs_results_gpt$gpt_health)
dim(all_imgs_results_gpt)

unique(all_imgs_results_gpt$gpt_image_reflection)
unique(all_imgs_results_gpt$gpt_image_clarity)
unique(all_imgs_results_gpt$gpt_health_confidence)
...
total_df = total_df %>% 
  filter(!is.na(gpt_health))
dim(total_df)

# keep only healths (both row and column wise) based on intersection of predictions
a = unique(total_df$gpt_health)
b = unique(total_df$label_health)

total_df$prediction_bigmodel <- health_cols[apply(total_df[,health_cols], 1, which.max)]
...
sum(unique_lables %in% health_cols) == length(unique_lables)

total_df = total_df %>% 
  filter(label_health %in% unique_lables,
         gpt_health %in% unique_lables,
         prediction_bigmodel %in% unique_lables)
dim(total_df)
total_df = total_df[, c("label", "group", "label_health", unique_lables, 
                        "gpt_health", "gpt_image_reflection", "gpt_image_clarity", 
                        "gpt_health_confidence", "avg_var_health", "avg_entropy_health")]
...
length(unique(total_df$gpt_health))
length(unique(total_df$label_health))
length(unique_lables)

total_df$X = 1:nrow(total_df)
total_df$true_label = total_df$label_health

# big model accuracy
sum(total_df$prediction_bigmodel == total_df$label_health) / nrow(total_df)
# gpt accuracy
sum(total_df$gpt_health == total_df$true_label) / nrow(total_df)
...
total_df = total_df %>% 
  mutate(gpt_health_confidence = case_when(gpt_health_confidence == "High" ~ 3,
                                           gpt_health_confidence == "Medium" ~ 2,
                                           gpt_health_confidence == "Low" ~ 1,
                                                 TRUE ~ 0),
         ...
         gpt_image_reflection = case_when(gpt_image_reflection == "High" ~ 3,
                                          ...
                                          TRUE ~ 0))
unique(total_df$gpt_health_confidence)
...
total_df = total_df %>% 
  left_join(label_idx_df, by=c("gpt_health"="original_label")) 
total_df = total_df %>% 
  mutate(gpt_health_encoded = label_idx) %>% 
  select(-label_idx)
unique(total_df$gpt_health_encoded)

total_df = total_df[order(total_df$gpt_health_confidence),]
...
total_df$gpt_correct = ifelse(total_df$gpt_health_encoded==total_df$true_label_encoded, TRUE, FALSE)
...
classificationB = total_df$gpt_health_encoded
truelabel = total_df$true_label_encoded

confidenceB = as.integer(total_df$gpt_health_confidence)
...
nConflevels = length(unique(total_df$gpt_health_confidence))
...
for(tmp in missingidx){
  ...
  tmp_gpt_label = total_df$gpt_health_encoded[tmp_idx]
