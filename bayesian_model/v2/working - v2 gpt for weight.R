...
###### BAYESIAN MODEL FOR WEIGHT #######
# process big model file
all_imgs_results_big_model = all_imgs_results_big_model3

# Select columns that start with 'Weight_'
weight_cols <- grep("Weight_", names(all_imgs_results_big_model), value = TRUE)
weight_cols <- sub("Weight_", "", weight_cols)
weight_cols <- gsub("\\.", "_", weight_cols)

# Extract true label for weights
all_imgs_results_big_model$label_weight <- sapply(strsplit(all_imgs_results_big_model$label, "_"), function(x) if(length(x) >= 2) x[2] else NA)
all_imgs_results_big_model$label_weight <- gsub("-", "_", all_imgs_results_big_model$label_weight)
table(all_imgs_results_big_model$label_weight)
sum(is.na(all_imgs_results_big_model$label_weight))

# Remove 'Weight_' prefix from column names
names(all_imgs_results_big_model) <- sub("Weight_", "", names(all_imgs_results_big_model))
names(all_imgs_results_big_model) <- sub("\\.", "_", names(all_imgs_results_big_model))

# Select label, group, logits and true label
all_imgs_results_big_model = all_imgs_results_big_model[,c("label","group",weight_cols,"label_weight", "avg_var_weight", "avg_entropy_weight")]
...
sum(is.na(all_imgs_results_gpt$gpt_weight)) > 0
sum(is.na(all_imgs_results_gpt$gpt_image_reflection)) > 0
sum(is.na(all_imgs_results_gpt$gpt_image_clarity)) > 0
sum(is.na(all_imgs_results_gpt$gpt_weight_confidence)) > 0

# keep valid weights
all_imgs_results_gpt$gpt_weight <- gsub("-", "_", all_imgs_results_gpt$gpt_weight)
unique(all_imgs_results_gpt$gpt_weight)
dim(all_imgs_results_gpt)
all_imgs_results_gpt = all_imgs_results_gpt[all_imgs_results_gpt$gpt_weight %in% weight_cols,]
unique(all_imgs_results_gpt$gpt_weight)
dim(all_imgs_results_gpt)

unique(all_imgs_results_gpt$gpt_image_reflection)
unique(all_imgs_results_gpt$gpt_image_clarity)
unique(all_imgs_results_gpt$gpt_weight_confidence)
...
total_df = total_df %>% 
  filter(!is.na(gpt_weight))
dim(total_df)

# keep only weights (both row and column wise) based on intersection of predictions
a = unique(total_df$gpt_weight)
b = unique(total_df$label_weight)

total_df$prediction_bigmodel <- weight_cols[apply(total_df[,weight_cols], 1, which.max)]
...
sum(unique_lables %in% weight_cols) == length(unique_lables)

total_df = total_df %>% 
  filter(label_weight %in% unique_lables,
         gpt_weight %in% unique_lables,
         prediction_bigmodel %in% unique_lables)
dim(total_df)
total_df = total_df[, c("label", "group", "label_weight", unique_lables, 
                        "gpt_weight", "gpt_image_reflection", "gpt_image_clarity", 
                        "gpt_weight_confidence", "avg_var_weight", "avg_entropy_weight")]
...
length(unique(total_df$gpt_weight))
length(unique(total_df$label_weight))
length(unique_lables)

total_df$X = 1:nrow(total_df)
total_df$true_label = total_df$label_weight

# big model accuracy
sum(total_df$prediction_bigmodel == total_df$label_weight) / nrow(total_df)
# gpt accuracy
sum(total_df$gpt_weight == total_df$true_label) / nrow(total_df)
...
total_df = total_df %>% 
  mutate(gpt_weight_confidence = case_when(gpt_weight_confidence == "High" ~ 3,
                                           gpt_weight_confidence == "Medium" ~ 2,
                                           gpt_weight_confidence == "Low" ~ 1,
                                                 TRUE ~ 0),
         ...
         gpt_image_reflection = case_when(gpt_image_reflection == "High" ~ 3,
                                          ...
                                          TRUE ~ 0))
unique(total_df$gpt_weight_confidence)
...
total_df = total_df %>% 
  left_join(label_idx_df, by=c("gpt_weight"="original_label")) 
total_df = total_df %>% 
  mutate(gpt_weight_encoded = label_idx) %>% 
  select(-label_idx)
unique(total_df$gpt_weight_encoded)

total_df = total_df[order(total_df$gpt_weight_confidence),]
...
total_df$gpt_correct = ifelse(total_df$gpt_weight_encoded==total_df$true_label_encoded, TRUE, FALSE)
...
classificationB = total_df$gpt_weight_encoded
truelabel = total_df$true_label_encoded

confidenceB = as.integer(total_df$gpt_weight_confidence)
...
nConflevels = length(unique(total_df$gpt_weight_confidence))
...
for(tmp in missingidx){
  ...
  tmp_gpt_label = total_df$gpt_weight_encoded[tmp_idx]
