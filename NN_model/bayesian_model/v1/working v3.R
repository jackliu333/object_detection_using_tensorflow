# ffmpeg -i path -vf fps=4 -q:v 2 $(date +'%Y%m%d')_5_%04d.jpg
...
total_df = total_df[, c("filepath", "label_prdtype", unique_lables, 
          "gpt_product_type", "gpt_image_reflection", "gpt_image_clarity", "gpt_prediction_confidence")]
dim(total_df)

##### adhoc adjustment - change some big model predictions into incorrect prediction
n_change = 100

# Function to introduce noise by switching the highest logit with another column's logit
add_noise <- function(df, n_rows) {
  nrows <- nrow(df)
  
  # Randomly select 'n_rows' rows to modify
  rows_to_modify <- sample(1:nrows, n_rows)
  
  for (row in rows_to_modify) {
    # Identify the column index of the highest logit for the current row
    highest_logit_col <- which.max(df[row, ])
    
    # Select a different column to switch with
    cols <- setdiff(1:ncol(df), highest_logit_col)
    random_col <- sample(cols, 1)
    
    # Switch the highest logit with the logit from the randomly selected column
    temp <- df[row, highest_logit_col]
    df[row, highest_logit_col] <- df[row, random_col]
    df[row, random_col] <- temp
  }
  
  return(df)
}

total_df[,unique_lables] = add_noise(total_df[,unique_lables], n_change)
...
test_idx1 = total_df$X[total_df$filepath %in% test_imgpaths][c(1,3,5,7,9)]
test_idx2 = sample(total_df$X[!(total_df$filepath %in% test_imgpaths)], size=as.integer(0.2*nrow(total_df)), replace=F)
