...
# Read CSV files
all_imgs_results_gpt <- read.csv("GPT_model/chatgpt_prediction.csv")
...
posterior_samples2 <- coda.samples(model_run$model, variable.names=model_params, n.iter=500)
summary(posterior_samples2)

# plot(posterior_samples2)
...
for(i in 1:nrow(total_df)){
  ...
  test_confidenceB = total_df$gpt_prediction_confidence[i]
  ...
  posterior_samples_new <- coda.samples(model_run_new$model, variable.names = c("truelabel"), n.iter = 500)
